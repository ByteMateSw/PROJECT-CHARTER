import React from 'react'
export default function home() {
  return (
    <div className="relative w-[1903] h-[3401px] relative bg-gradient-to-b from-[#FBFCFF] to-[#A1AEF7]">
    <div className='flex justify-start items-center'>
    <div className="w-[198px] h-[198px] left-[150px] top-[169px] absolute">
        <img src='./Figure5.svg'/>
        </div>/
    <div className="w-[120px] h-[120px] left-[249px] top-[604px] absolute">
        <img src='./Figure.svg'/>
    </div>
    <div className="w-[99px] h-[99px] left-[81px] top-[322px] absolute">
        <img src= "./Figure4.svg"/>
        </div>/
    <div className="w-[66px] h-[69px] left-[26px] top-[238.51px] absolute origin-top-left rotate-[-33.58deg]">
        <img src= "./figure9.svg"/>
        </div>
        <div className="w-[272px] h-[272px] left-[951px] top-[580.33px] absolute origin-top-left -rotate-45 justify-center items-center inline-flex">
            <img src="./Figure2.svg" alt='./vector2.svg'/>
        </div>
    <div className="w-[272px] h-[117.21px] left-[1168.23px] top-[-95px] absolute origin-top-left rotate-[40.57deg] justify-center items-center inline-flex">
        <img src= "./fig7.svg"/>
        </div> /
    <div className="w-[122.36px] h-[52.73px] left-[512.71px] top-[94px] absolute origin-top-left rotate-[12.83deg]">
        <img src ="./figure3.svg"/>
    </div>
    <div className="w-[148.39px] h-[154.06px] left-[740.93px] top-[637px] absolute origin-top-left rotate-45"/>
    <div className="w-[100px] h-[100px] left-[1821px] top-[44px] absolute">
        <img src= "./figure8.svg"/>
        </div>/
    <div className="w-[159px] h-[159px] left-[1637px] top-[826px] absolute">
        <img src = "./Figure6.svg"/>
        </div>/
    
         <div className="left-[16px] top-[194px] absolute flex-col justify-start items-start gap-12 inline-flex">
            <div className="text-black text-[64px] font-bold font-['Quicksand']">Haz realidad tus proyectos</div>
            <div className="text-black text-[64px] font-medium font-['Quicksand']">Conéctate con los mejores <br/>Encuentra talentos <br/>Contrata habilidades</div>
            </div>
       
   <div className='justify-end items-center'>
    <div className="w-[868.96px] h-[651px] left-[1172px] top-[194px] absolute">
        <div className="w-[868.96px] h-[657px] left-0 top-0 absolute">
            <img src="./group.svg"/>
        </div>
        <div className="w-[52.99px] h-[53.52px] left-[470.96px] top-[49.34px] absolute">
            <img src="./group1.svg"/>
        </div>
        <div className="w-[126.09px] h-[86.12px] left-[550.46px] top-[19.50px] absolute">
            <img src="./group2.svg"/>
        </div>
        <div className="w-[121.87px] h-[95.04px] left-[95.60px] top-[131.83px] absolute">
            <img src="./group3.svg"/>
        </div>
        <div className="w-[450.15px] h-[229px] left-[203.49px] top-[137.71px] absolute">
            <img src="./group10.svg"/>
         </div>
        <div className="w-[451.62px] h-[227.62px] left-[163.58px] top-[381.13px] absolute">
            <img src="./group11.svg"/>
        </div>
    </div>
    </div>
  
    </div>
    </div>
  )
}