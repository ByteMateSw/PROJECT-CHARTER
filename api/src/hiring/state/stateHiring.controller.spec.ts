...
describe('stateHiring', () =>{
    ...
    const mockCreateMessage = {message:'El estado del contrato ha sido creado correctamente'};
    const mockDeleteMessage = {message:'El estado del contrato se ha eliminado correctamente'};
    const mockUpdateMessage = {message:'El contrato se ha actualizado correctamente'};
    ...
    describe('createStatushire', () =>{
        ...
        it('should throw a HttpException for not creating a status Hire', async ()=>{
            const mockRequestBody= {name: 'newStatus'};
    ...
    describe('getStatusByName', () =>{
        ...
        it('should throw a HttpException for not getting a status Hire by Name', async ()=>{
            mockStateHiringService.getStatusByName.mockRejectedValue(mockError);
            await expect(async () => await controller.getStatusByName(mockStateHiring.name),
            ).rejects.toThrow(
                new HttpException(mockError.message, HttpStatus.BAD_REQUEST)
            );
        })
    ...
    describe('deleteStatusHire', () =>{
        it('should delete an status Hire', async() =>  {
            ...
            expect(mockStateHiringService.deleteStatusHire).toHaveBeenCalledWith(id)
        });
